...
def construct_adapter(model, data_provider, device,
                      enabled_methods=tuple(["p-conv", "p-lin", "q-fp32", "q-int8", "q-mixed"]),frozen_layers=None):
...
def compress_model(model: torch.nn.Module, policy_path, data_provider, device,
                   enabled_methods=tuple(["p-conv", "p-lin", "q-fp32", "q-int8", "q-mixed"]),frozen_layers=None):
    executable_model, policy = _compress_to_executable(data_provider, device, model, policy_path, enabled_methods,frozen_layers)
...
def _compress_to_executable(data_provider, device, model, policy_path,
    ...
    #_=[policy.layers.pop(k) for k in list(policy.layers) if 'model.22' in k]
    compression_adapter, model_ref = construct_adapter(model, data_provider, device, enabled_methods,frozen_layers)
