...
class IndependentController(AController):
    ...
    def search(self, number_of_episodes, model_reference):
        ...
        reference_evaluation = self._model_evaluator.evaluate(initial_model_for_metrics, list()) | self._lat_eval(
            initial_model_for_metrics) #evaluate acc and latency for uncompressed model
        ###
        #reference_train=self._model_evaluator.retrain(model_reference)
        ...
        # for normal (initial) reference policies this will have no compression effect (fp32 was chosen)
