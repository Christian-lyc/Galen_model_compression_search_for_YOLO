...
class TorchConfiguration:
    ...
    def __init__(self, target_device: torch.device, **kwargs):
        ...
        self.retrain_batch_size = int(kwargs.pop("retrain_batch_size", 2))#256
        self.retrain_epochs = int(kwargs.pop("retrain_epochs", 1))
        self.retrain_lr = float(kwargs.pop("retrain_lr", 0.01))
        self.retrain_mom = float(kwargs.pop("retrain_mom", 0.937))
        ...
        self.frozen_layers = ast.literal_eval(
            kwargs.pop("frozen_layers", "{'p-lin': ['fc'], 'q-mixed': ['fc','model.22.cv2.0.0.conv', "
                                        "'model.22.cv2.0.1.conv', 'model.22.cv2.0.2', 'model.22.cv2.1.0.conv', "
                                        "'model.22.cv2.1.1.conv', 'model.22.cv2.1.2', 'model.22.cv2.2.0.conv', "
                                        "'model.22.cv2.2.1.conv', 'model.22.cv2.2.2', 'model.22.cv3.0.0.conv', "
                                        "'model.22.cv3.0.1.conv', 'model.22.cv3.0.2', 'model.22.cv3.1.0.conv', "
                                        "'model.22.cv3.1.1.conv', 'model.22.cv3.1.2', 'model.22.cv3.2.0.conv', "
                                        "'model.22.cv3.2.1.conv', 'model.22.cv3.2.2', 'model.22.dfl.conv'],"
                                        "'p-conv':['model.22.cv2.0.0.conv', "
                                        "'model.22.cv2.0.1.conv', 'model.22.cv2.0.2', 'model.22.cv2.1.0.conv', "
                                        "'model.22.cv2.1.1.conv', 'model.22.cv2.1.2', 'model.22.cv2.2.0.conv', "
                                        "'model.22.cv2.2.1.conv', 'model.22.cv2.2.2', 'model.22.cv3.0.0.conv', "
                                        "'model.22.cv3.0.1.conv', 'model.22.cv3.0.2', 'model.22.cv3.1.0.conv', "
...
class TorchRecipe:
    ...
    def _get_controller(self, **args) -> AController:
        if self._agent.alg_mode() == AlgorithmicMode.ITERATIVE:
            return IterativeController(**args) #what is the difference
        return IndependentController(**args)

    def _get_reference_policy(self, compress_adapter):
        reference_policy = compress_adapter.get_reference_policy() #compression method for each layer
    ...
    def _create_data_provider(self, **kwargs):
        ...
        if self._config.data_set == "tick":
            return YOLODetectionProvider(**kwargs)
    ...
    def construct_application(self, torch_model: torch.nn.Module) -> tuple[AController, AModelReference]:
        ...
        model_factory = TorchModelFactory(torch_model=torch_model.model,
        ...
        sensitivity_analysis = SensitivityAnalysis(compress_adapter=compress_adapter,
                                                   ...
                                                   alg_mode=self._agent.alg_mode(), # SamplingPolicySupplier chosen
