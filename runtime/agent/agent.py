...
class ADdpgAgent(AAgent, metaclass=ABCMeta):
    ...
    def __init__(self, target_device: torch.device, agent_config=GenericAgentConfig()):
        ...
        if self._cfg.agent_action_seed:
            ...
            self._random_gen = np.random.Generator(np.random.PCG64()) #same as np.random.default_rng() but with controled seeds
    ...
    def do_prediction(self, global_step: GlobalStep, features: ModelFeatures,
        ...
        layer_state = self._extract_layer_state(target_layer, features) # preprocess state
        predicted_actions = self._predict_actions(global_step, layer_state) #prune ratio, weight quantization ratio, activation quantization ratio
        ...
        self._episode_protocol.append(self._current_episode_step) # add each layer
    ...
    def pass_episode_results(self, episode: int, episode_evaluation: dict[str, float],
                             final_observation: ModelFeatures) -> tuple[np.ndarray, namedtuple]:
        #rewards are a tensor for each layer
        ...
        for step_idx, protocol in enumerate(self._episode_protocol):
            ...
            self._append_to_experience_buffer(next_observation, protocol, rewards, step_idx) #relay buffer
