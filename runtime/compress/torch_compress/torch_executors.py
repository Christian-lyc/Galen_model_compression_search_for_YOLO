...
class TorchDepPruningExecutor(ATorchCompressionExecutor):
    ...
    def __init__(self, model_reference: TorchModelReference,
                 ...
                 method_key: str,
                 strategy=MagnitudeImportance,#tp.strategy.L1Strategy(),
                 prune_method=tp.function.prune_conv_out_channels,#
    ...
    def compress(self, compression_specs: dict[str, LayerCompressionSpecification],
        ...
        def _prune(l_key: str, c_spec: CompressionRatioParameter):
            ...
            weights = target_module.weight.data  # [out_channels, in_channels, kH, kW]
            importance_scores = torch.norm(weights, p=1, dim=[1,2,3])
            sparsity = amount_to_remove / len(importance_scores)
            n_keep = int(len(importance_scores) * (1 - sparsity))
            prune_idxs = torch.argsort(importance_scores)[:n_keep]

            all_channels = torch.arange(weights.shape[0])  # [0, 1, 2, ..., out_channels-1]
            prune_idxs = torch.tensor([i for i in all_channels if i not in prune_idxs])
            plan = dep_graph.get_pruning_group(
            target_module,
            self._prune_method,  # e.g., function.prune_conv_out_channels
            idxs=prune_idxs.tolist()
            )
            #prune_idxs = self._strategy(target_module.weight, amount_to_remove)
            #plan = dep_graph.get_pruning_group(target_module, self._prune_method, prune_idxs)
    ...
    def get_layer_compression_specification(self, layer_key) -> LayerCompressionSpecification:
        if layer_key in self._independent_prunable: # link to independent prunable layer
            ...
            return LayerCompressionSpecification.create_initial(layer_key, tuple([compression_parameter])) #default: is_active=False for compression
        return None
    ...
    def _remove_dependent_layers(self, prunable_keys: list[str]) -> set[str]:
        executable_model = self._model_factory.to_executable_model(self._model_reference) #copy of original model, and keep frozen layer as it is
        ...
        dep_graph.build_dependency(executable_model.pytorch_model,
                                   example_inputs=random_input[1,:,:,:].unsqueeze(0))
        ...
        return set(prunable_keys).difference(dependent_keys) #find independent layers' keys
    ...
    def _resolve_dependent_keys(self, dep_graph: tp.DependencyGraph(), layer_key: str, model: TorchExecutableModel) -> \
        ...
        weights = current_module.weight.data  # [out_channels, in_channels, kH, kW]
        importance_scores = torch.norm(weights, p=1, dim=[1,2,3])  # L1 norm per output channel

        # 2. Determine amount to prune (convert sparsity ratio to absolute number)
        sparsity = 0.5  # example: 50% pruning
        n_keep = int(len(importance_scores) * (1 - sparsity))
        prune_idxs = torch.argsort(importance_scores)[:n_keep]  # indices of channels to KEEP

        # 3. Get channels to REMOVE (invert selection)
        all_channels = torch.arange(weights.shape[0])  # [0, 1, 2, ..., out_channels-1]
        prune_idxs = torch.tensor([i for i in all_channels if i not in prune_idxs])
        pruning_plan = dep_graph.get_pruning_group(
            current_module,
            self._prune_method,  # e.g., function.prune_conv_out_channels
            idxs=prune_idxs.tolist()
        )
        ###old code
        #dummy_idxs = self._strategy(current_module.weight, amount=0.5) #percentage of pruned layers
        #pruning_plan = dep_graph.get_pruning_group(current_module, self._prune_method, idxs=dummy_idxs) #self._prune_method refer to output channels

        if not dep_graph.check_pruning_group(pruning_plan):
    ...
    def _extract_dependent_layer_names(self, pruning_plan) -> list[str]:
        dependent_keys = list()
        for dep_tuple in pruning_plan.items[1:]:#plan[1:]:  # skip first -> self dependency
            dependency = dep_tuple[0]
            # same handler type means this is a blocking dependency
            if isinstance(dependency.handler, type(self._prune_method)): #dependent layer to the pruned layer
    ...
    def _fetch_shapes(self, plan: tp.Group):
        shapes = dict()
        for dep_tuple in plan.items:
            dependency = dep_tuple.dep #dep_tuple[0]
...
class TorchMixedQuantizationExecutor(ATorchCompressionExecutor):
    ...
    def compress(self, compression_specs: dict[str, LayerCompressionSpecification],
        ...
        for layer_key, compression_spec in compression_specs.items():
            ...
            pre_hook = target_layer.register_forward_pre_hook(q_layer_context.pre_forward_hook) #pytorch function
            q_layer_context.set_pre_hook_handle(pre_hook)
            post_hook = target_layer.register_forward_hook(q_layer_context.post_forward_hook) #register hook will not run into it.
    ...
    def _extract_compressible_layers(self, model_reference: TorchModelReference,
        ...
        for supported_type in supported_layer_types:
            ...
            all_supported.update(no_frozen)
...
class TorchInt8QuantizationExecutor(TorchMixedQuantizationExecutor):
    ...
    def compress(self, compression_specs: dict[str, LayerCompressionSpecification],
        ...
        return super(TorchInt8QuantizationExecutor, self).compress(adapted_specs, executable_model) #to TorchMixedQuantizationExecutor
...
class TorchFp32Executor(TorchMixedQuantizationExecutor):
    ...
    def compress(self, compression_specs: dict[str, LayerCompressionSpecification],
        ...
        for layer_key in compression_specs.keys():
            ...
            if layer_key in quantization_contex: #Remove Existing Configuration
                quantization_contex.pop(layer_key)
        return list()

    def get_layer_compression_specification(self, layer_key) -> LayerCompressionSpecification:
        if self.is_quantizable(layer_key): #check quantization list
            return LayerCompressionSpecification.create_initial(layer_key, default_active_state=True)
        return None

    def _is_layer_compatible(self, layer_key, model_info) -> bool:
        return True #overrides _is_layer_compatible function in TorchMixedQuantizationExecutor
