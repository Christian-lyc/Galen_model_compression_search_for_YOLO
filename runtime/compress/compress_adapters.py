...
class ACompressAdapter(metaclass=ABCMeta):
    ...
    def do_compress(self, policy: CompressionPolicy, model_reference: AModelReference) -> (
        ...
        policy.verify_mutual_groups() #verify only one is active of different pruning or quantization methods
        discretized_policy = copy.deepcopy(policy)
        if not self._no_discretization: #by default quantization discretization is none
        ...
        for key, executor in self._executors.items():
            compression_specs = discretized_policy.get_specs_for_key(key) #read key related compression specification
    ...
    def get_reference_policy(self) -> CompressionPolicy:
        layer_specifications = OrderedDict.fromkeys(self._model_reference.all_layer_keys(), dict()) #store compression details
        for definition in self._compression_methods:
            new_layer_specifications = self._get_layer_specifications_for_method(definition) #remove complex dependency from the prunable list
    ...
    def _get_layer_specifications_for_method(self, definition: CompressionMethodDefinition) -> dict[
        ...
        for layer_key in all_layers:
            if executor.is_supported(layer_key): #test if a layer support this compression method
                layer_specifications[layer_key] = executor.get_layer_compression_specification(layer_key) #depend on method link to quantization executor or pruning executor
